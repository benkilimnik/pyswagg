...
A clone of pyswagr with a couple bug fixes.

[![Build Status](https://travis-ci.org/mission-liao/pyswagr.svg?branch=master)](https://travis-ci.org/mission-liao/pyswagr)
[![Coverage Status](https://coveralls.io/repos/mission-liao/pyswagr/badge.svg?branch=master&style=flat)](https://coveralls.io/r/mission-liao/pyswagr?branch=master)
...
**pyswagr** is much easier to use (compared to swagger-codegen, you don't need to prepare a scala environment) and tries hard to **fully supports** [Swagger Spec](https://helloreverb.com/developers/swagger) in all aspects.
...
#### report an issue:
- issues can be reported [here](https://github.com/mission-liao/pyswagr/issues)
- include swagger.json if possible
- turn on logging and report with messages on console
```python
import logging
logger = logging.getLogger('pyswagr')
...
unit testing
```bash
python -m pytest -s -v --cov=pyswagr --cov-config=.coveragerc pyswagr/tests
```
